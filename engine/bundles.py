...
def capabilities_data(bundles: dict[str, Bundle] = BUNDLES) -> list[dict]:
    """Machine-readable bundle catalog (for ``capabilities --json``).

    The markdown table is for humans; this compact form is for the LLM/tooling
    (fewer tokens than parsing markdown).
    """
    return [
        {
            "id": b.id,
...
def render_capabilities_markdown(bundles: dict[str, Bundle] = BUNDLES) -> str:
    ...
    lines = [
        ...
        "",
        "Machine-readable (compact, for LLM/tooling): "
        "`python3 -m engine.cli capabilities --json`.",
        "",
